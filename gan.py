...
for step in range(nSteps):
    ...
    artist_paintings = data.dist_batches
    G_ideas = np.random.randn(batch, num_gen_seeds)
    G_paintings, pa0, Dl, _, _ = \
        sess.run([gen_out, prob_origin, dis_cost, dis_train_op, gen_train_op], feed_dict={gen_in: G_ideas, original_dist: artist_paintings})
